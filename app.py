...
from forms import UserAddForm, UserUpdateForm, LoginForm, MessageForm
...
def like_message(message_id):
    ...
    if not msg.user_id == g.user.id and not msg in g.user.likes:
        ...
        flash("You've already liked that message. Something has gone wrong.", "warning")
        return redirect("/")
    else:
...
def unlike_message(message_id):
    ...
    if msg in g.user.likes:
        ...
        flash(
            "You can't unlike a message you haven't already liked. Something has gone wrong.",
            "warning",
        )
        return redirect("/")
...
def homepage():
    ...
    if g.user:
        following_ids = [followed_user.id for followed_user in g.user.following] + [
            g.user.id
        ]
        messages = (
